...
class FactoryBase {
  ...
  static _getCollectionJsonLD () {
    if (!this.collectionJsonLD) {
      this.collectionJsonLD = this._getJsonLD('collections')
    }
    return this.collectionJsonLD
  }
  ...
  static async _getJsonLD (filename) {
    ...
    if (!request) {
      ...
      const url = `https://raw.githubusercontent.com/NYPL/nypl-core/${coreVersion()}/vocabularies/json-ld/${filename}.json`
